...
const ApiEvents = class {
  ...
  add (event) {
    ...
    return Event.findOrCreate(conditions, event).then(result => {
      if (result[1]) {
  ...
  edit (hash, data) {
    ...
    return Event.findOneAndUpdate(conditions, data, options).lean().exec().then(result => {
      if (result) {
  ...
  delete (hash) {
    ...
    const conditions = { hash };
    return Event.findOneAndRemove(conditions).lean().exec().then(result => {
      if (result) {
