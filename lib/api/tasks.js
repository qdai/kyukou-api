...
const ApiTasks = class {
    ...
    return runTaskAndSave('twit_new', async () => {
      ...
      const postEvent = async event => {
        ...
        try {
          ...
          return event.updateOne({ 'tweet.new': true }).exec();
        } catch (err) {
          /* istanbul ignore next */
          return err;
        }
  ...
  twitTomorrow () {
    return runTaskAndSave('twit_tomorrow', async () => {
      ...
      const postEvent = async event => {
        ...
        try {
          await this.twitter.post(text);
          return event.updateOne({ 'tweet.tomorrow': true }).exec();
        } catch (err) {
          /* istanbul ignore next */
          return err;
        }
  ...
  delete () {
    return runTaskAndSave('delete', async () => {
      ...
      const removed = await Event.find(conditions).deleteMany();
      return `msg: ${removed.deletedCount} event(s) deleted`;
    });
  }
};
