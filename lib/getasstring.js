function get (event) {
  event.datetime = (event.eventDate.getMonth() + 1) + '月'
...
get.prototype = {
  ...
  asNewTweet: function () {
    ...
    var text = '新規：【' + event.about + '】' + event.datetime + '\n'
  ...
  asTomorrowTweet: function () {
    var event = this.source;
    var text = '明日：【' + event.about + '】' + event.datetime + '\n'
             + event.department + event.period + '時限' + event.fullsubject + '\n';
    if (event.room) {
      text += '教室：' + event.room + '\n';
    }
  ...
  asRSSTitle: function () {
    var event = this.source;
    var text = '【' + event.about + '】' + event.datetime
