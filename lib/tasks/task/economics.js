...
const config = {
  baseURL: 'http://www.econ.kyushu-u.ac.jp',
  resourcePath: '/student/kyuukou.php'
};

module.exports = () => {
  return fetch(config.baseURL + config.resourcePath).then($ => {
    return $('.box01 tr[bgcolor="#FFFFFF"]').map((i, el) => {
      const $item = $(el);
      const data = {};
