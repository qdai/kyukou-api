...
async.parallel([
    ...
    fetch(config.resource.education, 'utf-8', function (err, res, $) {
      ...
      list.each(function () {
        try {
          ...
          console.log('err: unknown format: %s', data.raw.replace(/[\f\n\r]/g, ''));
          console.log('msg: %s', e);
        }
      });
      var tmpE = [];
      ...
      list.each(function () {
        try {
          ...
          data.eventDate = $(this).find(':nth-child(2)').text().replace(/[Ａ-Ｚａ-ｚ０-９]/g, function (s) {
            return String.fromCharCode(s.charCodeAt(0) - 65248);
          }, '').replace(/(^\s+)|(\s+$)/g, '').replace(/年|月/g, '-0').replace(/日.*/, '');
          if (data.eventDate.match(/-(\d*)-/)[1].length === 3) {
            data.eventDate.replace(/-0(\d*)-/, '-$1-');
          } else if (data.eventDate.match(/-(\d*)$/)[1].length === 3) {
            data.eventDate.replace(/-0(\d*)$/, '-$1');
          }
  ...
  async.each(events, function (event, eachCallback) {
    Event.findOrCreate({
      ...
      if (err) {
        console.log('err: findorcreate failed on ' + event.raw.replace(/[\f\n\r]/g, ''));
        console.log('msg: %s', err);
        return eachCallback();
      }
      if (created) {
